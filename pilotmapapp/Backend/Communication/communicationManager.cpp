...
void communicationManager::setupIPCSocket(){
    ...
    if (zmq_connect(ipcSub, addr.c_str()) == -1)
...
bool communicationManager::reconnect(void* s, std::string addr){
    if (zmq_disconnect(s, lastIPCAddress.c_str()) == -1)
        qDebug() << "Failed to disconnect from lastIPC Address " << QString::fromStdString(lastIPCAddress);
    
    std::string newAddr = "tcp://" + addr + ":5556";
    if (zmq_connect(s, newAddr.c_str()) == -1){
        qCritical("Failed to connect ipcSub");
        return false;
    }
